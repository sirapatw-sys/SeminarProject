...
- **เสียงของห้องเป็นข้อมูล:** `RoomKnowledgeData.sounds` คือรายการเสียงวน (คลิป + ความดัง) ของแต่ละห้อง `SfxPlayer` crossfade ให้เองเมื่อเปลี่ยนห้อง เสียงที่ห้องใช้ร่วมกัน (ฝน) ไม่เริ่มใหม่ แค่ค่อยๆ ปรับความดัง
  - Room01: `Room01 BGM 1` + ฝนเบามาก / Room02: `Room02 BGM 1` + `Grandpa Clock 1` เบาๆ + ฝนเบามาก / Room03: ฝนเบา + เสียงเบสตุบๆ ช้าๆ ดังคงที่ และเสียงสุ่มในห้องทุก 40–90 วิ (`hauntedDrone`) จากคลิปใน `Assets/Resources/Audio/Noises/` (เสียงหลอน 7 เสียง + เคาะประตู 2 แบบ ปรับให้ดังเท่ากัน) ของที่ตั้ง `scareOnSuccess` มีโอกาส 35% เรียกเสียงสุ่มให้ดังตามมาเร็วขึ้น NPC ที่มี `startleLines` ใน profile (ตอนนี้มีแค่สเตล) จะสะดุ้งกับเสียงสุ่มเป็นบางครั้ง
  ...
  - `InteractionData.successSound` = เสียงสั้นที่ดังจากในห้องเมื่อทำสำเร็จ (เด็กหัวเราะที่เตาผิง) ไม่หรี่เสียงห้อง
  - `Assets/Resources/Audio/Door` เล่นทุกครั้งที่เดินผ่านประตูไปห้องถัดไป
...
ผู้เล่นเห็นค่าเป็น ♥ x/100 ในกล่องบทสนทนา ค่าแต่ละช่วงกำหนดใน `tones` ของ profile: น้ำเสียงที่ AI ต้องใช้ และระดับคำใบ้สูงสุด (ไม่ใบ้ / กว้างๆ / ปกติ / ชัดเจน)

| NPC | เริ่ม | ช่วง → พฤติกรรม | ความลับ |
|---|---|---|---|
| Alice | 50 | ≤35 เคือง เย็นชา ใบ้กว้างๆ / 36–65 วางตัวนิ่ง ใบ้ปกติ / 66–80 เปิดใจ ใบ้ชัด / 81+ สนิทมาก เป็นห่วง | ≥70 เรื่องน้องชาย |
| Sena | 30 | ไม่สนใจความสัมพันธ์ ไม่ใบ้ทุกระดับ | ≥80 รอยขีดใต้ฝาหีบเป็นของนาง |
| Rina | 45 | ≤35 เย็นชาเต็มที่ ใบ้กว้างๆ / 36–60 สุภาพห่างๆ ใบ้ปกติ / 61–80 ไว้ใจขึ้น ใบ้ชัด / 81+ ไว้ใจเต็มที่ | ≥65 กลัวผี, ≥75 เริ่มลืมนามสกุล |
| Stelle | 35 | ≤45 ขี้อาย ติดอ่าง ไม่ใบ้ / 46–60 เริ่มไว้ใจ ใบ้กว้างๆ / 61–80 สนิท ขี้อ้อน ใบ้ปกติ / 81+ กล้าเล่าสิ่งที่กลัว | ≥55 รู้จักเพลงกล่อมเด็ก, ≥65 เคยเห็นเด็กในกระจก |

ค่าเปลี่ยนจาก (ตัวเลขทั้งหมดอยู่ใน `Assets/Scripts/Core/RelationshipTuning.cs`):
- ข้อความที่พิมพ์ — AI ตอบทั้ง `playerTone` และตัวเลข แล้วโค้ดบังคับตัวเลขให้อยู่ในช่วงของ tone นั้น: hostile -15 ถึง -5, cold -8 ถึง -2, neutral 0 ถึง +1, friendly +1 ถึง +6, kind +4 ถึง +10
- ตัวอ่านน้ำเสียงแบบกฎ (`PlayerToneClassifier`) อ่านทั้งประโยค: ประชด, ปฏิเสธ ("ไม่ได้ว่าเธอ"), "แต่" (สิ่งที่พูดทีหลังชนะ), คำติที่พูดใส่ NPC vs ใส่ห้อง/ตัวเอง แล้ว `RelationshipTuning.Judge` รวมกับคำตัดสินของ AI:
  - กฎว่าหยาบ + AI กลางๆ/ไม่มี AI → ไม่เกิน -5 (hostile) / -2 (cold)
  - กฎว่าหยาบ + AI ว่าเป็นมิตร → ขัดกัน ได้ 0 (ไม่ให้รางวัลสิ่งที่อาจหยาบ และถ้ากฎพลาดก็เสียแค่คะแนนที่ควรได้ ไม่ถูกหัก)
  - กฎว่าเป็นมิตร + AI กลางๆ → อย่างน้อย +1/+2 · กฎว่าเป็นมิตร + AI ว่าหยาบ → เชื่อ AI (ประชดที่กฎไม่เห็น)
  - วัดกับ AI จริงแล้ว (terra, 396 ประโยค): ระบบรวมถูก 97% ก่อนปรับ prompt ไม่มีประโยคหยาบได้คะแนนเพิ่ม จุดอ่อนของ AI คือให้คะแนนประโยคกลางๆ เกินจริง ซึ่ง prompt รอบล่าสุดแก้ไปส่วนใหญ่
  - ชุดทดสอบ 1,520 ประโยคอยู่ที่ `Assets/Tests/Editor/ToneCorpus/` (NEG/NEU/POS) เทสต์บังคับว่าไม่มีประโยคดี/กลางๆ ถูกอ่านว่าหยาบ ไม่มีประโยคหยาบถูกอ่านว่าดี และถูก ≥99% — ถ้าเพิ่มคำ ให้เพิ่มประโยคลงไฟล์ด้วย
- คำตอบสำรองตอนไม่มี AI — ค่าบวกที่เขียนไว้ใน profile คูณ 2
- ตัวเลือกในบทสนทนาและ event — ค่าบวกที่เขียนไว้คูณ 1.5 (ปัดขึ้น) ค่าลบคงเดิม
...
- `freeTopic` = AI เลือกเรื่องที่จะคุยเอง (`AiDialogueGenerator.BuildFreeTopicPrompt`) ตัวเลือกใน DialogueData เป็นแค่ "ท่าที" ที่ AI ต้องคงลำดับ ผลของตัวเลือกยังมาจาก DialogueData ถ้า AI ใช้ไม่ได้หรือเขียนไม่สำเร็จจะไม่ขึ้น "!"
...
| Alice/Sena/Rina/Stelle_Chatter | ทุกตัว | สุ่ม (`freeTopic`: AI เลือกหัวข้อเองจากสิ่งที่ตัวละครรู้/จำได้/รู้สึก) หลังเคยคุยกันแล้ว ต้องมี AI | ท่าทีของตัวเลือก: ตั้งใจฟัง +3 และ memory / ถามกลับ +1 / ปัดไป −2 |

"!" แบบสุ่ม: ทุก 5 วินาที NPC แต่ละตัวที่ยังไม่มี "!" ค้างทอยโอกาส 50% แล้วสุ่มเลือกจาก event ที่เงื่อนไขครบและพ้น cooldown (120–180 วิ) "!" อยู่ 40–45 วิแล้วหายถ้าไม่มีใครมาคุย story beat ที่ถึงเวลาจะแทนที่ "!" ธรรมดาที่ค้างอยู่
...
- หน้าตั้งค่า (`AiSettingsPanel`) เลือกโมเดลจากรายการ KKU ที่ติดมากับเกม 21 ตัว (กดโหลดรายชื่อล่าสุดได้) พร้อมผลทดสอบกับ prompt ของเกม (26 ก.ย. 2026): แนะนำ terra, luna, claude-sonnet-5, gemini 3 ตัว, mistral-medium-3, grok-4.5, nova 2 ตัว; qwen และ kimi-k3 ช้าเกินเวลารอ 20 วิ; deepseek, llama, mistral-small, minimax ตอบเป็นข้อความธรรมดาแทน JSON บ่อย (ลอง `response_format: json_object` แล้ว KKU ไม่ช่วย)
- ปุ่มในหน้าต่าง IMGUI แบบ modal ใช้ `ModalGui` (วาดหน้าสุด ทำงานตอนกด กินคลิกที่เหลือ) เพราะ `GUI.Button` พลาดคลิกเมื่อหลายหน้าต่างซ้อนกัน
- โมเดลเริ่มต้นของ KKU คือ `gpt-5.6-terra` (`AiDialogueGenerator.DefaultKkuModel`) ค่าที่เคยบันทึกเป็น luna จะถูกย้ายมาเป็น terra ครั้งเดียว ถ้าเลือก luna ใหม่ในหน้าตั้งค่าจะจำไว้ตามนั้น KKU ระบุว่าทั้ง luna และ terra เป็นของ OpenAI และมีโควตา 200,000 token/วัน/โมเดล/key (ตอบกลับมาในช่อง `model_quota`)
...
- คำขอแบบ chat completions (KKU/Gemini/custom) ของ event บอกรูปแบบ JSON เป๊ะ (`CompatibleDialogueFormat`) เพราะถ้าไม่บอก โมเดลตั้งชื่อช่องเองและเกมจะใช้บทสำรองทุกครั้ง
